...
async function main() {
  ...
  // 1. Criar ou obter o Tenant Padrão "venusta"
  let tenantDaVinci = await prisma.tenant.findUnique({
    where: { subdomain: 'venusta' },
  });
  ...
  if (!tenantDaVinci) {
    ...
    console.log('Tenant padrão "venusta" criado.');
  }

  // 2. Criar ou obter o Super Admin
  const superAdminEmail = 'superadmin@appvenusta.com.br';
  ...
  if (serviceCount === 0) {
    ...
    const coloracao = await prisma.service.create({
      data: { nome: 'Coloração & Mechas Venusta', preco: 280.0, duracao: 120, tenantId: tenantDaVinci.id },
    });
