...
export default function WhatsAppSimulator() {
  ...
  const { data: barbers = [] } = useQuery({
    ...
    enabled: whatsappConn?.status === 'CONNECTED',
  });
  ...
  // WebSockets listener
  useEffect(() => {
    if (whatsappConn?.status !== 'CONNECTED') return;
  ...
  }, [selectedClientId, queryClient, whatsappConn?.status]);
  ...
  const isConnecting = whatsappConn?.status === 'CONNECTING';
          ...
          {isConnecting ? (
            <div className="space-y-5 w-full flex flex-col items-center">
              ...
              <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-inner flex items-center justify-center w-48 h-48 relative">
                {qrCodeData && qrCodeData !== 'MOCK_QR_CODE_DATA' ? (
                  <img
