...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Plus, CheckCircle, Play, Smile, XCircle, Search, Lock, Unlock } from 'lucide-react';
...
export default function DashboardCalendar() {
  ...
  const [blockBarberId, setBlockBarberId] = useState<string | null>(null);
  const [blockTitle, setBlockTitle] = useState('Bloqueio Manual');
  const [isUnblockModalOpen, setIsUnblockModalOpen] = useState(false);
  const [selectedBlockForRemoval, setSelectedBlockForRemoval] = useState<any>(null);
  
  // Drag and Drop states
  const [draggedOverCell, setDraggedOverCell] = useState<{ hour: number; barberId: string } | null>(null);
  ...
  const createBlockMutation = useMutation({
    mutationFn: ({ barberId, block }: { barberId: string; block: any }) =>
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/barbers/${barberId}/blocks`, {
        method: 'POST',
        headers: {
        ...
        if (!res.ok) {
          throw new Error(data?.message || 'Não foi possível bloquear o horário.');
        }
        return data;
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allAgendaBlocks'] });
      addNotification({
        title: 'Horário bloqueado',
        description: 'O horário foi bloqueado com sucesso na agenda.',
        type: 'success',
      });
      setIsBlockModalOpen(false);
      setBlockTitle('Bloqueio Manual');
      setBlockHour(null);
      setBlockBarberId(null);
    },
    onError: (err: any) => {
      addNotification({
  ...
  const deleteBlockMutation = useMutation({
    mutationFn: (blockId: string) =>
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/barbers/blocks/${blockId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
        ...
        const data = await res.json().catch(() => null);
        if (!res.ok) {
          throw new Error(data?.message || 'Não foi possível desbloquear o horário.');
        }
        return data;
      }),
    onSuccess: () => {
      ...
      setSelectedBlockForRemoval(null);
    },
    onError: (err: any) => {
      addNotification({
  ...
  const rescheduleMutation = useMutation({
    mutationFn: ({ id, barberId, data }: { id: string; barberId: string; data: Date }) =>
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/appointments/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({ barberId, data: data.toISOString() }),
      }).then(async (res) => {
        const resData = await res.json().catch(() => null);
        if (!res.ok) {
          throw new Error(resData?.message || 'Erro ao reagendar atendimento.');
        }
        return resData;
      }),
    onSuccess: () => {
      ...
      addNotification({
        ...
        description: 'O atendimento foi movido com sucesso.',
        type: 'success',
      });
    },
    onError: (err: any) => {
      addNotification({
        ...
        description: err.message || 'Verifique a disponibilidade e tente novamente.',
        type: 'error',
      });
    },
  });
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'SCHEDULED':
        return 'border border-davinci-gold/30 bg-davinci-gold/5 text-davinci-gold shadow-[0_2px_8px_rgba(0,0,0,0.04)]';
      case 'CONFIRMED':
        return 'border border-blue-200 bg-blue-50 text-blue-700 shadow-[0_2px_8px_rgba(0,0,0,0.04)]';
      case 'CHECKED_IN':
        return 'border border-amber-200 bg-amber-50 text-amber-800';
      case 'IN_PROGRESS':
        return 'border-2 border-emerald-500 bg-emerald-50 text-emerald-800 animate-pulse';
      case 'COMPLETED':
        return 'bg-gold-gradient text-davinci-black font-semibold border-none shadow-[0_2px_8px_rgba(0,0,0,0.08)]';
      case 'CANCELLED':
  ...
  const handleAppointmentDrop = (appointmentId: string, targetHour: number, targetBarberId: string) => {
    const appointment = appointments.find((app: any) => app.id === appointmentId);
    if (!appointment) return;

    // Se dropou na mesma posição (mesmo profissional e mesma hora), não faz nada
    const appDate = new Date(appointment.data);
    if (appointment.barberId === targetBarberId && appDate.getHours() === targetHour) {
      return;
    }

    // Verificar se o local de destino está bloqueado
    const isBlocked = agendaBlocks.some((block: any) => {
      ...
      return block.barberId === targetBarberId && targetTime >= blockStart && targetTime < blockEnd;
    });

    if (isBlocked) {
      addNotification({
        title: 'Horário indisponível',
        description: 'Este horário está bloqueado para o profissional.',
        type: 'warning',
      });
      return;
    }

    // Verificar se já existe outro agendamento no mesmo horário/profissional
    const hasConflict = dailyAppointments.some((app: any) => {
      const d = new Date(app.data);
      return app.id !== appointmentId && d.getHours() === targetHour && app.barberId === targetBarberId;
    });
  ...
  return (
    <div className="space-y-6">
                      ...
                      <div className="grid" style={{ gridTemplateColumns: `repeat(${filteredBarbers.length || 1}, minmax(0, 1fr))` }}>
                        {filteredBarbers.length === 0 ? (
                          ...
                          filteredBarbers.map((barber: any) => {
                            ...
                            const isHoveredCol = hoveredCell?.barberId === barber.id;
                            const isDraggedOver = draggedOverCell?.hour === hour && draggedOverCell?.barberId === barber.id;
                            
                            const hasConflict = dailyAppointments.some((app: any) => {
                              const d = new Date(app.data);
                              return app.id !== isDragging && d.getHours() === hour && app.barberId === barber.id;
                            });

                            const isCellBlocked = !!cellBlock;
                            const isValidDropTarget = !isCellBlocked && !hasConflict;
                            ...
                            return (
                              <div
                                ...
                                onDragEnter={(e) => {
                                  ...
                                  e.preventDefault();
                                  setDraggedOverCell({ hour, barberId: barber.id });
                                }}
                                onDragOver={(e) => {
                                  if (!canReschedule) return;
                                  e.preventDefault();
                                ...
                                onDrop={(e) => {
                                  ...
                                  if (appointmentId) {
                                    handleAppointmentDrop(appointmentId, hour, barber.id);
                                  }
                                }}
                                onClick={() => {
                                  if (cellBlock) {
                                    setSelectedBlockForRemoval(cellBlock);
                                ...
                                className={`p-2 flex flex-col justify-center h-[72px] cursor-pointer transition-all relative group ${
                                  isDraggedOver
                                    ? isValidDropTarget
                                      ? 'bg-davinci-gold/20 border border-dashed border-davinci-gold/60 scale-[0.97]'
                                      : 'bg-red-500/15 border border-dashed border-red-500/40 scale-[0.97]'
                                    : isHoveredCell
                                    ? 'bg-davinci-gold/15'
                              ...
                              {cellAppointment ? (
                                <div
                                  ...
                                  onDragStart={(e) => {
                                    if (!canReschedule) return;
                                    setIsDragging(cellAppointment.id);
                                    e.dataTransfer.setData('text/plain', cellAppointment.id);
                                  ...
                                  className={`w-full h-full ${
                                    canReschedule
                                      ? isDragging === cellAppointment.id
                                        ? 'cursor-grabbing opacity-40 select-none'
                                ...
                                >
                                  <motion.div
                                    ...
                                    className={`w-full h-full p-2.5 rounded-lg text-left text-xs flex flex-col justify-between shadow-sm border-l-4 transition-all relative ${getStatusStyle(
                                      cellAppointment.status
                                    )}`}
                                    style={{
                                      borderLeftColor: cellAppointment.status === 'SCHEDULED' ? 'var(--primary-color, #C5A880)' :
                                  ...
                                  >
                                    <div className="font-bold text-davinci-black truncate flex items-center gap-1">
                                      <span className="truncate">{cellAppointment.client.nome}</span>
                                      {cellAppointment.payWithCredit && (
                                        <span className="px-1 py-0.2 rounded bg-purple-100 border border-purple-200 text-purple-700 text-[7px] font-black uppercase tracking-tight shrink-0" title="Assinatura Ativa">
                                          Club
                                        </span>
                                      )}
                                    </div>
                                    <div className="flex items-center justify-between text-[9px] mt-1 font-semibold text-davinci-gray">
                                      <span className="truncate max-w-[60%]">{cellAppointment.service.nome}</span>
                                      {cellAppointment.payWithCredit ? (
                                        <span className="text-purple-600 font-extrabold">Assinatura</span>
                                      ) : (
                                        <span className="text-davinci-black font-extrabold">R$ {cellAppointment.valor}</span>
                                      )}
                                    </div>
                                    <span className="absolute top-1.5 right-1.5 text-[8px] uppercase tracking-wider font-extrabold px-1.5 py-0.5 rounded bg-current/10 text-current">
                                      {getStatusLabel(cellAppointment.status)}
                                    </span>
                                  </motion.div>
                                </div>
                              ) : cellBlock ? (
                                <div className="w-full h-full p-2.5 rounded-lg bg-zinc-100 border border-zinc-200/80 text-zinc-500 text-left flex flex-col justify-between transition-all hover:bg-red-50 hover:border-red-200 hover:text-red-600 relative overflow-hidden group/block">
                                  <div className="flex flex-col justify-between h-full group-hover/block:opacity-0 transition-opacity">
                                    <div className="font-bold text-[10px] uppercase tracking-wider truncate flex items-center gap-1">
                                      ...
                                      <span className="truncate">{cellBlock.titulo}</span>
                                    </div>
                                    <span className="text-[8px] uppercase tracking-wider font-extrabold opacity-60">
                                      Bloqueado
                                    </span>
                                  </div>
                                  <div className="absolute inset-0 flex items-center justify-center bg-red-500/10 text-red-600 opacity-0 group-hover/block:opacity-100 transition-opacity text-xs font-bold uppercase tracking-wider gap-1.5">
                                    <Unlock className="h-4 w-4" />
                                    Desbloquear
                                  </div>
                                </div>
                              ) : (
                                <div className="w-full h-full flex items-center justify-center border border-dashed border-zinc-200/60 rounded-lg relative overflow-hidden group/empty">
                                  <Plus className="h-4.5 w-4.5 text-zinc-300 group-hover/empty:opacity-0 transition-opacity" />
                                  <div className="absolute inset-0 flex items-center justify-center gap-2 p-1.5 opacity-0 group-hover/empty:opacity-100 bg-white/90 transition-opacity">
                                    <button
                                      type="button"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                      ...
                                      className="flex-1 h-full max-h-[36px] bg-davinci-gold text-white rounded-md text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1 hover:opacity-90 transition-opacity cursor-pointer shadow-sm border-none"
                                    >
                                    ...
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setBlockHour(hour);
                                        setBlockBarberId(barber.id);
                                        setBlockTitle('Bloqueio Manual');
                                        setIsBlockModalOpen(true);
                                      }}
                                      className="flex-1 h-full max-h-[36px] bg-zinc-900 text-white rounded-md text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1 hover:bg-zinc-800 transition-colors cursor-pointer shadow-sm border-none"
                                    >
            ...
            >
              ...
              <form onSubmit={handleCreateAppointment} className="p-6 space-y-5 overflow-y-auto">
                ...
                <button
                  ...
                  className="w-full py-3 bg-gold-gradient rounded-lg text-white font-bold text-sm hover:scale-[1.01] active:scale-[0.99] transition-all shadow-[0_4px_15px_rgba(0,0,0,0.12)] cursor-pointer disabled:opacity-60"
                >
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-background p-4 rounded-xl border border-zinc-200/80 space-y-2 text-xs text-davinci-gray font-semibold">
                  ...
                  <div className="flex justify-between">
                    <span>Valor:</span>
                    {selectedAppointment.payWithCredit ? (
                      <span className="text-purple-600 font-extrabold">Coberto por Assinatura</span>
                    ) : (
                  ...
                  {selectedAppointment.payWithCredit && (
                    <div className="flex justify-between">
                      ...
                      <span className="px-2 py-0.5 rounded bg-purple-100 border border-purple-200 text-purple-700 text-[10px] font-bold">Assinante do Clube</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full p-3 bg-gold-gradient text-white disabled:opacity-40 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 shadow-[0_4px_10px_rgba(0,0,0,0.1)] cursor-pointer"
                  >
      ...
      {/* MODAL 3: Bloquear Horário */}
      <AnimatePresence>
        {isBlockModalOpen && blockHour !== null && blockBarberId && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-zinc-200 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col"
            >
              <div className="p-6 border-b border-zinc-200/80 bg-background flex justify-between items-center">
                <h3 className="text-md font-bold text-davinci-black uppercase tracking-wider flex items-center gap-2">
                  <Lock className="h-5 w-5 text-davinci-gold" />
                  Bloquear Horário
                </h3>
                <button
              ...
              <div className="p-6 space-y-5">
                <div className="bg-background p-3 rounded-lg border border-zinc-200/80 flex items-center justify-between text-xs text-davinci-gray font-semibold">
                  <span>Profissional: <strong className="text-davinci-black">{barbers.find((b: any) => b.id === blockBarberId)?.user.nome}</strong></span>
                  <span>Horário: <strong className="text-davinci-black">{`${blockHour.toString().padStart(2, '0')}:00`}</strong></span>
                </div>

                <div className="space-y-2">
                  <label className="block text-xs font-bold text-davinci-gray uppercase tracking-wider">
                  ...
                  <input
                    ...
                    placeholder="Ex: Almoço, Reunião, Ausente..."
                    className="w-full px-4 py-2.5 bg-white border border-zinc-200 rounded-lg text-davinci-black focus:outline-none focus:border-davinci-gold text-sm"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsBlockModalOpen(false)}
                    className="flex-1 py-2.5 border border-zinc-200 text-davinci-black hover:bg-zinc-50 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    onClick={() => {
                      ...
                      dataInicio.setHours(blockHour, 0, 0, 0);
                      const dataFim = new Date(selectedDate);
                      dataFim.setHours(blockHour + 1, 0, 0, 0);

                      createBlockMutation.mutate({
                        barberId: blockBarberId,
                        block: {
                          titulo: blockTitle.trim() || 'Bloqueio Manual',
                    ...
                    className="flex-1 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white disabled:opacity-60 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                  >
      ...
      {/* MODAL 4: Remover Bloqueio (Desbloquear) */}
      <AnimatePresence>
        {isUnblockModalOpen && selectedBlockForRemoval && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              ...
              <div className="p-6 space-y-5">
                <p className="text-xs text-davinci-gray leading-relaxed font-medium">
                  Tem certeza que deseja remover o bloqueio <strong className="text-davinci-black">"{selectedBlockForRemoval.titulo}"</strong>? Este horário voltará a ficar disponível para agendamento dos clientes no catálogo e via WhatsApp.
                </p>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      deleteBlockMutation.mutate(selectedBlockForRemoval.id);
                    }}
                    disabled={deleteBlockMutation.isPending}
                    className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white disabled:opacity-60 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <Unlock className="h-4 w-4" />
                    {deleteBlockMutation.isPending ? 'Removendo...' : 'Remover Bloqueio'}
                  </button>
                </div>
              </div>
            </motion.div>
