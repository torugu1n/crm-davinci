...
export default function ReportsManager() {
  ...
  return (
    <div className="space-y-6">
      {/* Estilos customizados para impressão PDF */}
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          /* Esconder elementos de layout, navegação e botões */
          .no-print,
          aside,
          header,
          #saas-paywall,
          ...
          input,
          select,
          .bg-white.p-4.rounded-xl.border {
            display: none !important;
          }
          
          /* Forçar os contêineres de layout principais a fluírem naturalmente */
          html, body, 
          .min-h-screen, 
          #main-content-wrapper,
          main,
          #dashboard-tab-content-reports {
          ...
          * {
          ...
          /* Ajustar cabeçalho de impressão para ser mais compacto */
          .print-header {
            margin-bottom: 0.75rem !important;
            padding-bottom: 0.5rem !important;
          }

          /* Remover sombras e bordas excessivas e reduzir paddings para compactação */
          .bg-white {
            border: 1px solid #e4e4e7 !important;
            box-shadow: none !important;
            background: #fff !important;
            page-break-inside: avoid !important;
            padding: 0.75rem !important; /* Reduzir padding na impressão para caber tudo em 1 página */
            border-radius: 0.75rem !important;
          }
          
          /* Reduzir espaçamentos verticais na impressão */
          .space-y-6 > * + * {
          ...
          /* KPI Cards em 4 colunas na impressão para economizar espaço vertical */
          .grid-kpis {
            grid-template-columns: repeat(4, 1fr) !important;
          }

          /* Reduzir valores de KPIs e margem mt-4 */
          .text-2xl {
          ...
          /* Limitar altura máxima do gráfico SVG para garantir que caiba em 1 página */
          svg {
            max-width: 100% !important;
            height: auto !important;
      ...
      {/* Cabeçalho exclusivo para impressão em PDF */}
      <div className="print-header hidden print:block mb-8 border-b-2 border-[#C5A880] pb-4">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-2xl font-black text-zinc-900 uppercase tracking-wider">
              {tenant?.name || 'Venusta'}
            </h1>
            <p className="text-[10px] text-[#C5A880] font-bold uppercase tracking-widest mt-0.5">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] text-zinc-500 font-bold mt-1">
              Período: {startDate ? formatFullDate(startDate) : ''} até {endDate ? formatFullDate(endDate) : ''}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-zinc-200/85 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="no-print px-4 py-2 bg-davinci-black hover:bg-davinci-gold text-white font-semibold rounded-lg text-xs transition-all flex items-center gap-2 cursor-pointer shadow-sm hover:scale-102 active:scale-98"
          >
            <Printer className="h-4 w-4 text-davinci-gold" />
      ...
      {!isLoading && !error && activeSubTab === 'financial' && reportData && (
        <div className="space-y-6 animate-fade-in">
          {/* Financial Cards */}
          <div className="grid-kpis grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              {
