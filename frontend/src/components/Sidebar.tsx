...
export default function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  ...
  const allMenuItems = [
    ...
    ...(tenant?.subscriptionModuleEnabled ? [{ id: 'subscriptions', label: 'Clube de Assinatura', icon: CreditCard }] : []),
    { id: 'reports', label: 'Relatórios', icon: BarChart3 },
    { id: 'finance', label: 'Financeiro', icon: DollarSign },
    { id: 'users', label: 'Usuários e Acessos', icon: Key },
    ...
    <>
      ...
      <aside
        className={`w-64 bg-white border-r border-zinc-200/80 flex flex-col h-screen fixed left-0 top-0 z-40 lg:z-20 transition-transform duration-300 ${
          mobileMenuOpen ? 'translate-x-0 pointer-events-auto' : '-translate-x-full lg:translate-x-0 pointer-events-none lg:pointer-events-auto'
        }`}
      >
        {/* Brand Logo */}
        <div className="p-6 border-b border-zinc-200/80 flex items-center gap-3 shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[8px] text-davinci-gold uppercase tracking-[0.05em] font-medium mt-0.5 leading-tight break-words">
              {tenant?.name ? 'Estabelecimento Parceiro' : 'CRM & Gestão Inteligente'}
            </p>
          </div>
        </div>
