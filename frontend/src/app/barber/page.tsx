    ...
    <div className="min-h-screen bg-background text-davinci-black p-6 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-davinci-gold/10 border border-davinci-gold/30 rounded-full text-davinci-gold animate-spin-slow">
            <Scissors className="h-6 w-6" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-davinci-black">{barber.nome}</h2>
            <p className="text-[10px] text-davinci-gold font-bold uppercase tracking-wider">
        ...
        <button
          ...
          className="p-2 rounded-lg bg-white border border-red-200 text-red-500 hover:bg-red-50 transition-colors text-xs font-bold flex items-center gap-1.5 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-between h-24">
          <span className="text-[9px] text-davinci-gray uppercase tracking-widest font-bold">Atendimentos</span>
          <h3 className="text-xl font-bold text-davinci-black mt-1">{metrics.totalAppointments}</h3>
        </div>

        <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-between h-24">
          <span className="text-[9px] text-davinci-gray uppercase tracking-widest font-bold">Faturamento Total</span>
          <h3 className="text-xl font-bold text-davinci-black mt-1">R$ {metrics.totalBilling.toFixed(2)}</h3>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-between h-24">
          ...
          <h3 className="text-xl font-bold text-davinci-black mt-1">{metrics.returnRate}%</h3>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
        ...
        {achievements.length === 0 ? (
          <p className="text-xs text-davinci-gray font-semibold italic">Continue atendendo clientes para desbloquear medalhas corporativas.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {achievements.map((ach, idx) => (
              <div key={idx} className={`p-4 rounded-xl bg-gradient-to-br ${ach.color} text-white flex flex-col justify-between space-y-2 shadow-sm`}>
                <span className="text-xs font-bold block">{ach.title}</span>
                <span className="text-[9px] opacity-90">{ach.description}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="space-y-4">
        ...
        {todayAppointments.length === 0 ? (
          <div className="bg-white p-8 rounded-2xl border border-zinc-200 shadow-sm text-center text-xs text-davinci-gray font-semibold italic">
            Nenhum agendamento marcado para hoje.
          </div>
        ) : (
          <div className="space-y-4">
            {todayAppointments.map((app: any) => (
              <div key={app.id} className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="text-sm font-bold text-davinci-black">{app.client.nome}</h4>
                    <p className="text-[10px] text-davinci-gold mt-1 font-bold uppercase tracking-wider">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider ${
                    app.status === 'COMPLETED'
                      ...
                      : 'bg-zinc-100 text-davinci-black'
                  }`}>
                    {app.status}
                  </span>
                </div>
                ...
                <div className="bg-background p-3 rounded-lg border border-zinc-200/60 flex justify-between items-center">
                  <div className="text-xs text-davinci-gray flex-1">
                    ...
                    <p className="mt-1 leading-relaxed italic text-[11px] text-davinci-black font-semibold">
                  ...
                  <button
                    ...
                    className="ml-4 px-3 py-1.5 rounded-lg bg-white hover:bg-davinci-gold/10 text-davinci-gold border border-davinci-gold/20 hover:border-davinci-gold/50 text-[10px] font-bold transition-all cursor-pointer shadow-sm"
                  >
                    Notas & Estilo
                  </button>
                </div>
                ...
                {activeClientNotesId === app.client.id && (
                  <div className="bg-background p-4 rounded-lg border border-zinc-200 space-y-4">
                    <h5 className="text-[10px] font-bold text-davinci-black uppercase tracking-widest">
                      Atualizar Ficha de Estilo: {app.client.nome}
                    </h5>
                    
                    <div className="space-y-3 text-xs">
                      <div>
                        <label className="block text-[9px] text-davinci-gray uppercase tracking-wider mb-1 font-bold">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-zinc-200 focus:border-davinci-gold rounded-lg px-3 py-2 text-davinci-black focus:outline-none"
                        />
                      </div>
                    ...
                    <div className="flex justify-end gap-2 text-xs">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg text-davinci-gray hover:text-davinci-black cursor-pointer font-bold"
                      >
                        Cancelar
                      </button>
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-gold-gradient text-white font-bold flex items-center gap-1 cursor-pointer shadow-sm hover:scale-[1.01]"
                      >
