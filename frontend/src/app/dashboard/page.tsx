...
export default function DashboardPage() {
  ...
  const allowedTabs = user ? getAllowedDashboardTabs(user) : [];
  ...
  // Sincroniza a aba quando a URL muda (ex: botões avançar/voltar do navegador)
  // Nota: Não incluímos 'activeTab' no array de dependências para evitar que o efeito reverta
  // temporariamente o estado local para o valor da URL antiga enquanto o Next.js realiza a transição assíncrona.
  useEffect(() => {
    if (!user) return;
    const allowed = getAllowedDashboardTabs(user);
    if (allowed.length > 0) {
      if (!requestedTab || !allowed.includes(requestedTab as any)) {
        const defaultTab = allowed[0] || 'calendar';
  ...
  // Supabase Realtime para notificações globais em tempo real no Dashboard
  useEffect(() => {
    if (!token) return;
    ...
    const channel = supabase
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            addNotification({
              ...
              description: 'Um novo agendamento foi adicionado à agenda.',
              type: 'success',
            });
          }
        }
      ...
      .on(
        ...
        (payload) => {
          ...
          addNotification({
            title: 'Novo Feedback',
            description: `Nova avaliação recebida com nota ${payload.new.nota}.`,
            type: 'info',
          });
        }
      )
        ...
        (payload) => {
          if (payload.new.whatsapp_status !== payload.old.whatsapp_status) {
            ...
            addNotification({
              ...
              description: `Conexão do WhatsApp alterada para ${payload.new.whatsapp_status}.`,
              type: payload.new.whatsapp_status === 'CONNECTED' ? 'success' : 'warning',
            });
          }
        }
      )
  ...
  return (
    <div className="min-h-screen bg-background text-davinci-black font-sans flex">
      ...
      {/* Main Container */}
      <div id="main-content-wrapper" className="flex-1 lg:pl-64 flex flex-col min-h-screen">
        {/* Top Header */}
        <div className="no-print">
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          ...
          {activeTab === 'users' && isAdminUser(user) && (
            <div
              ...
              data-demo-title="Usuários e Acessos"
          ...
          {activeTab === 'subscriptions' && isAdminUser(user) && (
            <div id="dashboard-tab-content-subscriptions">
